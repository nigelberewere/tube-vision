...
export function extractYouTubeError(errorPayload: any, fallbackMessage: string, fallbackStatus = 500) {
  ...
  const reason =
    typeof errorPayload?.error?.errors?.[0]?.reason === "string"
...
const YOUTUBE_DATA_CACHE_SECRET =
  process.env.YOUTUBE_CACHE_SECRET || process.env.SESSION_SECRET || "tube-vision-secret";
...
async function writeYouTubeDataCache(params: {
  ...
  const ttlSeconds = Math.max(
    30,
    Math.min(3600, Number(params.ttlSeconds) || YOUTUBE_DATA_CACHE_DEFAULT_TTL_SECONDS),
  );
...
export function pickBestTopicInsight(signals: CoachVideoSignal[]): CoachTopicInsight | null {
  ...
  for (const token of candidateTokens) {
    ...
    if (baseline.length < 3) {
      baseline = signals.filter((signal) => !recentMatches.some((item) => item.id === signal.id)).slice(0, 3);
    }
    ...
    if (recentRetentionValues.length >= 2 && baselineRetentionValues.length >= 2) {
      ...
      if (baselineRetentionAverage > 0) {
        retentionLiftPercent =
          ((recentRetentionAverage - baselineRetentionAverage) / baselineRetentionAverage) * 100;
        usesRetention = true;
      }
    }
  ...
  const bestLift = best.usesRetention && best.retentionLiftPercent !== null ? best.retentionLiftPercent : best.liftPercent;
...
export function getSessionAccountsAndActiveIndex(req: express.Request) {
...
export function setSessionAccountsAndActiveIndex(req: express.Request, accounts: any[], activeIndex: number) {
  const session = req.session as any;
